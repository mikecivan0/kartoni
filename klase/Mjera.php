...
class Mjera extends SQL{
	...
	public function setBolesnaStrana($bolesnaStrana){
		$this->bolesnaStrana = $bolesnaStrana;
	}
	...
	public function isMesurements(){
		...
		$izraz = $veza->prepare("select * from kartoni_mjere where osoba_id=:osoba_id;");
	...
	public function insert(){
		...
		$osoba_id = $this->osoba_id;
		$datum = $this->datum;
		$bolesnaStrana = $this->bolesnaStrana;
		...
		$izraz->bindParam(":osoba_id", $osoba_id);
		$izraz->bindParam(":datum", $datum);
		$izraz->bindParam(":bolesnaStrana", $bolesnaStrana);
	...
	public function ispisMjera($doktorica=false){
		...
		if(!empty($this->isMesurements())){
			...
			//dohvati sve mjere
			$izraz = $veza->prepare("select * from kartoni_mjere where osoba_id=:osoba_id order by datum desc;");
			$izraz->bindParam(":osoba_id", $osoba_id);
			$izraz->execute();		
